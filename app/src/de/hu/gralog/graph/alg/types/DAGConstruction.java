...
import de.hu.gralog.graph.alg.types.CopsAndRobberAlgorithm.CopsAndRobberVertex;
import de.hu.gralog.graph.alg.types.CopsAndRobberAlgorithm.DummyVertexFilter;
...
public class DAGConstruction<V extends LabeledGraphVertex,E extends DefaultEdge> extends Algorithm {
	...
	public AlgorithmResult execute(  ) throws InvalidPropertyValuesException {
		...
		if ( graph.createVertex() instanceof CopsAndRobberVertex ) {
			...
			result.setSingleContent( new AlgorithmResultContent( getDAG( (DirectedGraph<CopsAndRobberVertex, E>) graph ) ) );
			return result;
		}
	...
	public DirectedGraph<LabeledGraphVertex, DefaultEdge> getDirectedGraphFromDAG( DirectedGraph<DAGVertex, DefaultEdge> dag ) {
		DirectedGraph<LabeledGraphVertex, DefaultEdge> graph = new DirectedGraph<LabeledGraphVertex, DefaultEdge>( LabeledGraphVertex.class );
	...
	public <VI extends CopsAndRobberVertex<V>, EI extends DefaultEdge> DirectedGraph<DAGVertex, DefaultEdge> getDAG( DirectedGraph<VI, EI> gameGraph ) {
		Set<VI> winningPlayer0 = Algorithms.FiniteGameAlgorithm.getWinningPositionsPlayer0( gameGraph );
	...
	private static class Player0WinningVertexFilter implements VertexFilter {
		private Set win;
